...
// ─── Text renderer ─────────────────────────────────────────────────────────────
// Blank chips only fill in after verification.
...
function renderPassageText(
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (blankMatch) {
      ...
      const qIdx = blankNum - 1;

      if (verified) {
        const selected = currentAnswers[qIdx];
        const correct = questions[qIdx]?.answer;
        const isCorrect = selected === correct;
        const optionIdx = OPTION_KEYS.indexOf(selected as "A" | "B" | "C" | "D");
        const optionText =
          questions[qIdx]?.options[optionIdx]?.replace(/^\([A-D]\)\s*/, "") ??
          selected ??
          "—";
        result.push(
          <span
            key={`blank-${i}`}
            className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 mx-0.5 text-sm font-semibold border ${
              isCorrect
                ? "bg-green-50 text-green-700 border-green-300"
                : "bg-red-50 text-red-700 border-red-300"
            }`}
          >
            <span className="text-sm opacity-60">[{blankNum}]</span>
...
export default function ExoPart6Shell({
  passages,
  advice,
  exerciseLabel,
  exerciseKey,
  ...
  // Mutable selections for the current passage — changeable until verified
  const [currentAnswers, setCurrentAnswers] = useState<Record<number, string>>({});
  const [verified, setVerified] = useState(false);
  // Locked answers per passage (saved on verify)
  ...
  function handleVerify() {
    ...
    setAllAnswers((prev) => ({
      ...prev,
      [currentPassageIndex]: currentAnswers,
    }));
    document.querySelector("main")?.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  function getLetterBadgeStyle(questionIdx: number, key: string): string {
    ...
    if (!verified) {
      if (currentAnswers[questionIdx] === key)
        return `${base} bg-[#7c3aed] text-white`;
      ...
      <div className="p-8 md:p-10">
        ...
        <div className="inline-flex items-center gap-2 rounded-full border border-[#ddd6fe] bg-[#f3eeff] px-4 py-1.5 mb-5">
          ...
          <span className="text-xs text-[#9f7aea]">
            · {totalPassages} textes · {totalPassages * 4} questions
          </span>
        </div>
